"""
    Final Version of the Animation Creator
    Date:   Mid June 
    Purpose:The purpose of this program is to take pressure values and 
            create an animation of the COP with that data. 
    Notes:  This program reads the Results.xlsx file will go through 
            every point in that excel sheet and produce an animation for
            Every iteration. These animations will be stored in a folder
...
filePath = 'Results.xlsx'                   # this is the file path of the Excel sheet
sheet=2                                     # Usually 2
folderName='newVideos'                  # Name of the folder to save the animations in
...
def getIterationArray(iterationIndex):              #returns a 2D array with Index,Pinky and thumbData
    temp = ''
    increaseIndex = 3       #index that the values start at
    indexArr = []               #Empty arrays
    pinkyArr = []
    thumbArr = []

    while pd.isnull(temp) == False:             #While the value in the Excel collumn is not Nan, reads from top to bottom
        temp = list_data[increaseIndex][iterationIndex]      #index collum
        temp2 = list_data[increaseIndex][iterationIndex+1]     #pinky collum
        temp3 = list_data[increaseIndex][iterationIndex+2]     #thumb collum
        indexArr.append(temp)               #append each index value to the indexArray list
        pinkyArr.append(temp2)              #append each pinky value to the pinkyArray list
        thumbArr.append(temp3)              #append each thumb value to the thumbArray list
    ...
    threeDataPoints.append(indexArr)
...
def getCogCoordniates():
    ...
    for i in range(len(TwoDArray[1])):         #for loop, runs the length of the list
        ...
        if (force[0] + force[1] + force[2]) == 0:                      #If forces are all 0
            ...
            xPoint = (indexCoord[0] * force[0] + pinkyCoord[0] * force[1] + thumbCoord[0] * force[2]) / (force[0] + force[1] + force[2])
            yPoint = (indexCoord[1] * force[0] + pinkyCoord[1] * force[1] +  thumbCoord[1] * force[2]) / (force[0] + force[1] + force[2])
###############################Flag above
...
if(os.path.exists(filePath)):
    ...
    for i in  range( len(list_data[0]) -1):
        ...
        saveName = f"{folder_name}/Animation"
        ...
        ani = animation.FuncAnimation(fig, update, frames=len(lines) + 1, interval=200, repeat=False)       
